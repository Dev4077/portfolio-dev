...
import { saveSkillAction, deleteSkillAction } from "@/app/actions/admin";
...
export default function AdminSkillsClient({ initialData }: { initialData: any[] }) {
  const [skills] = useState(initialData);
  ...
  const onSubmit = async (values: z.infer<typeof skillSchema>) => {
    ...
    const result = await saveSkillAction(editingId, values);
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="flex justify-end space-x-2">
                  ...
                  <Button type="submit" disabled={isSubmitting}>Save Skill</Button>
                </div>
              </form>
            </Form>
          </DialogContent>
              ...
              skills?.map((skill) => (
                <TableRow key={skill.id} className="border-b border-white/5 border-opacity-50">
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end space-x-2">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(skill)} className="text-muted-foreground hover:text-primary">
                        <Edit className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleDelete(skill.id)} className="text-muted-foreground hover:text-destructive">
